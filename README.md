...
A promotions engine for WooCommerce, built as an engineering sample - AI-first
(Claude Code) in one working day, every line reviewed and tested by the author.
The admin creates promotions (five discount types), the plugin applies them during
cart calculation, shows them on the site (deals page, popup with countdown,
mini-cart, checkout) and tracks their performance in a built-in analytics screen.
...
3. Load the demo promotions: **Promo Engine -> Promotions -> "Load demo promotions"**
   ...
   **SAVE10** (-10%) used by reference case 7.

## Demo promotions and category mapping

The requirements use abstract "Category 1/2/3"; on the demo site they are mapped to real
categories:

| Requirement | Real category (slug) | Promotion |
|------|----------------------|-----------|
...
The Flash Sale product list deliberately includes Shorts products so that reference
case 5 (a flash-discounted item inside a BOGO group) is reproducible on the
live site.
...
```
php tests/engine-test.php     # 38 assertions, includes all 9 reference cases
```

The pipeline follows the documented calculation order:
...
fee): reference case 7 requires `SAVE10` to yield $184.68 from $205.20 - i.e. the
...
The requirement says "if at least one competing promotion is non-stacking -> apply only
the highest-priority one", but its own reference case 5 requires the *non-stacking*
...
### Other decisions left open by the requirements
...
- **Cap** (`max discount %`): bounds the *total* discount produced by a stage on
  ...
  promos apply, the strictest (lowest) cap among them wins - reference case 8
  (-50% x -60% -> capped at -70%) works exactly this way.
...
- **BOGO unit selection**: merchant-safe - the globally cheapest eligible units
  become the free ones (`floor(units / (X+Y))` groups). Matches both reference
  cases; with e.g. 4 eligible units the cheapest 3 form the group and the most
...
- **Partial-line prices**: when only part of a line's quantity is discounted
  ...
  averaged unit price.
- **Sale prices**: discounts always start from the current (sale) price, as required.

### Promotions storage & admin
...
### Frontend - all in the plugin, theme untouched
...
- Scope note: the cart/mini-cart/checkout UI targets the classic (shortcode)
  WooCommerce surfaces - that is what the standard WooCommerce hooks (mini-cart
  fragments, review-order rows) exist for, and what the demo site's theme uses.
...
- **Checkout savings breakdown** and **standalone unit tests** for the
  calculation (38 assertions incl. all 9 reference cases) - see above.
...
| Feature | Where |
|------|-------|
| Admin - CRUD, all 5 types, demo seeding (button + WP-CLI) | `includes/admin/` |
| Engine - ordering, combination, cap, sale-aware, no recursion | `includes/engine/`, `includes/cart/` |
| Frontend - /deals/, popup, mini-cart, checkout breakdown | `includes/frontend/` |
| Analytics - events table, analytics screen, SQL aggregation, SVG chart | `includes/analytics/`, `includes/admin/class-analytics-page.php` |
| Reference cases 1-9 | `tests/engine-test.php` (all green) |
| Additional: checkout breakdown, usage limit, unit tests, popup A/B | all four included |
